...
chatbot_re = (r'((?:spot|spotify)\s(.+)|(.+)\s(?:spot|spotify))', re.I)
@hook.regex(*chatbot_re)
@hook.command
def chatbot(inp, reply=None):
    thought = inp.group(1).replace('spotify','').replace('spot','').replace('ALICE','Uguu').strip()
    response = botsession.think(thought)
    reply(response.replace('Bartender Katie','Uguu').replace('ALICE','UGUU~~~').replace('Daisy Picnic','ihatevim').replace('ELVIS','ihatevim').replace('<br> ','').replace('German Alice','urmom').replace('Dancing Alice','ihatevim').replace('Www.AliceBot.Org','spotify.com').replace('alicebot','spotify')  )
