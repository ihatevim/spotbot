...
fractal_counter = 0
hexafluoride_counter = 0

@hook.command(autohelp=False)
def fractal(inp):
    global fractal_counter
    return 'fractal has said "faggot" {} times today.'.format(fractal_counter)

@hook.command(autohelp=False)
def hexafluoride(inp):
    global hexafluoride_counter
    return 'hexafluoride has said "windows" {} times today.'.format(hexafluoride_counter)
...
def sieve_suite(bot, input, func, kind, args):
    ...
    if 'fractal' in input.nick.lower(): fractal_counter+=input.msg.lower().count('faggot')

    if 'hexafluoride' in input.nick.lower(): hexafluoride_counter+=input.msg.lower().count('windows')
